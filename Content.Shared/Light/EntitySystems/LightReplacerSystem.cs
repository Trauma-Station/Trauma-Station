...
public sealed partial class LightReplacerSystem : EntitySystem
{
    ...
    public bool TryReplaceBulb(Entity<LightReplacerComponent?> replacer, Entity<PoweredLightComponent?> lightHolder, EntityUid? userUid = null)
    {
        ...
        var wasReplaced = _poweredLight.ReplaceBulb(lightHolder, insertedBulb.Value, out var oldBulb, lightHolder.Comp); // Trauma - addedGoobstation - Recycle bulbs!
        if (wasReplaced)
        {
            ...
            if (oldBulb == null)
                return true;
            PredictedDel(oldBulb);
            replacer.Comp.GlassRecycled += replacer.Comp.GlassPerBulb;

            if (replacer.Comp.GlassRecycled >= replacer.Comp.GlassRequired)
            {
                replacer.Comp.GlassRecycled -= replacer.Comp.GlassRequired;
                _provider.AddToProvider(replacer.Owner, replacer.Comp.LightBulbProto, userUid);
            }
            Dirty(replacer);
            // </Trauma>
        }
