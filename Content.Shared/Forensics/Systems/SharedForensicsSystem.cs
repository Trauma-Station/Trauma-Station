...
public sealed partial class SharedForensicsSystem : EntitySystem
{
    ...
    private void OnBeingGibbed(Entity<ForensicsComponent> ent, ref GibbedBeforeDeletionEvent args)
    {
        ...
        foreach (var part in args.Giblets)
        {
            ...
            partComp.DNAs.Add((dna, TimeSpan.Zero)); // Trauma - add freshness timespan
    ...
    private void OnMeleeHit(Entity<ForensicsComponent> weapon, ref MeleeHitEvent args)
    {
        ...
        foreach (var hitEntity in args.HitEntities)
        {
            ...
            // <Trauma> - freshness info
            {
                ...
                weapon.Comp.DNAs.RemoveWhere(x => x.Item1 == hitEntityComp.DNA);

                weapon.Comp.DNAs.Add((hitEntityComp.DNA, _timing.CurTime));
            }
            // </Trauma>
        }
        Dirty(weapon);
    ...
    {
        List<(string, TimeSpan)> list = []; // Trauma - add TimeSpan
    ...
    public List<(string, TimeSpan)> GetSolutionsDNA(Solution soln) // Trauma - add TimeSpan
    {
        ...
        foreach (var reagent in soln.Contents)
        {
            ...
            {
                if (data is DnaData dna) // Trauma - cast properly
                {
                    list.Add((dna.DNA, dna.Freshness)); // Trauma - add Freshness data, use dna cast from above
                }
            }
        }
        return list;
    ...
    private void OnCleanForensicsDoAfter(Entity<ForensicsComponent> component, ref CleanForensicsDoAfterEvent args)
    {
        ...
        // <Trauma>
        targetComp.Scent = string.Empty; // TODO: move this to using a shower
    ...
    public string GenerateFingerprint(
        byte length = 16) // Trauma
    {
        var fingerprint = new byte[(int) length]; // Trauma - use length param
    ...
    public void TransferDna(EntityUid recipient, EntityUid donor, bool canDnaBeCleaned = true)
    {
        ...
        // <Trauma>
        if (!recipientComp.CanStoreDNA)
            return;
        recipientComp.DNAs.Add((donorComp.DNA, TimeSpan.Zero)); // added freshness timespan
        // </Trauma>
