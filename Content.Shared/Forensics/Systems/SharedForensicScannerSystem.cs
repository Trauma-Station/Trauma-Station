...
public sealed partial class SharedForensicScannerSystem : EntitySystem
{
    ...
    private void OnDoAfter(Entity<ForensicScannerComponent> scanner, ref ForensicScannerDoAfterEvent args)
    {
        ...
        {
            scanner.Comp.SolutionDNAs.Clear(); // Trauma
            ...
            {
                scanner.Comp.SolutionDNAs.AddRange(_forensicsSystem.GetSolutionsDNA(args.Args.Target.Value)); // Trauma - solution instead of touch dna
            }
            ...
            DirtyFields(scanner.AsNullable(),
                null,
                nameof(ForensicScannerComponent.SolutionDNAs), // Trauma
    ...
    private void OnPrint(Entity<ForensicScannerComponent> scanner, ref ForensicScannerPrintMessage args)
    {
        ...
        // <Trauma> - replace DNAs with Touch/Solution using freshness
        foreach (var (dna, freshnessTimestamp) in scanner.Comp.DNAs)
        {
        ...
        foreach (var (dna, freshnessTimestamp) in scanner.Comp.SolutionDNAs)
        {
            if (scanner.Comp.DNAs.Contains((dna, freshnessTimestamp)))
                continue;
            var timePassed = _gameTiming.CurTime - freshnessTimestamp;
            if (timePassed < TimeSpan.FromMinutes(1f))
            {
