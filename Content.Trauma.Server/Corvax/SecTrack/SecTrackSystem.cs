...
namespace Content.Trauma.Server.Corvax.SecTrack;

public sealed partial class SecTrackSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SecTrackComponent, MapInitEvent>(OnMapInit);
        SubscribeLocalEvent<SecTrackComponent, ActivatableUIOpenAttemptEvent>(OnUIOpenAttempt);
        SubscribeLocalEvent<SecTrackComponent, BoundUIOpenedEvent>(OnUIOpened);

        SubscribeLocalEvent<SecTrackComponent, CreateSquadMessage>(OnCreateSquad);
        SubscribeLocalEvent<SecTrackComponent, DeleteSquadMessage>(OnDeleteSquad);
        SubscribeLocalEvent<SecTrackComponent, RenameSquadMessage>(OnRenameSquad);
        SubscribeLocalEvent<SecTrackComponent, ChangeSquadIconMessage>(OnChangeSquadIcon);
        SubscribeLocalEvent<SecTrackComponent, UpdateSquadDescriptionMessage>(OnUpdateSquadDescription);
        SubscribeLocalEvent<SecTrackComponent, AddMemberToSquadMessage>(OnAddMemberToSquad);
        SubscribeLocalEvent<SecTrackComponent, RemoveMemberFromSquadMessage>(OnRemoveMemberFromSquad);
        SubscribeLocalEvent<SecTrackComponent, ChangeSquadStatusMessage>(OnChangeSquadStatus);
        SubscribeLocalEvent<SecTrackComponent, RemoveTimerMessage>(OnRemoveTimer);
    ...
    public override void Update(float frameTime)
    {
        ...
        if (currentTime - _lastSensorUpdate >= TimeSpan.FromSeconds(5))
        {
            ...
            var query = EntityQueryEnumerator<SecTrackComponent>();
    ...
    private void UpdateSensorStatuses(EntityUid uid, SecTrackComponent comp)
    {
        ...
        if (!_ui.IsUiOpen(uid, SecTrackUiKey.Key))
            return;

        var statusUpdate = new SensorStatusUpdateState(statusDict, squadLocations);
        _ui.SetUiState(uid, SecTrackUiKey.Key, statusUpdate);
    }
    ...
    private void OnMapInit(Entity<SecTrackComponent> ent, ref MapInitEvent args)
    {
    ...
    private void OnUIOpenAttempt(Entity<SecTrackComponent> ent, ref ActivatableUIOpenAttemptEvent args)
    {
        if (ent.Comp.Station == null)
            args.Cancel();
    }

    private void OnUIOpened(Entity<SecTrackComponent> ent, ref BoundUIOpenedEvent args)
    {
    ...
    private void OnCreateSquad(EntityUid uid, SecTrackComponent component, CreateSquadMessage msg)
    {
    ...
    private void OnDeleteSquad(EntityUid uid, SecTrackComponent component, DeleteSquadMessage msg)
    {
    ...
    private void OnRenameSquad(EntityUid uid, SecTrackComponent component, RenameSquadMessage msg)
    {
        if (component.Station == null || !_stationData.TryGetValue(component.Station.Value, out var stationData)
    ...
    private void OnChangeSquadIcon(EntityUid uid, SecTrackComponent component, ChangeSquadIconMessage msg)
    {
        if (component.Station == null || !_stationData.TryGetValue(component.Station.Value, out var stationData))
            return;
    ...
    private void OnUpdateSquadDescription(EntityUid uid, SecTrackComponent component, UpdateSquadDescriptionMessage msg)
    {
        if (component.Station == null || !_stationData.TryGetValue(component.Station.Value, out var stationData))
            return;
    ...
    private void OnAddMemberToSquad(EntityUid uid, SecTrackComponent component, AddMemberToSquadMessage msg)
    {
        if (component.Station == null || !_stationData.TryGetValue(component.Station.Value, out var stationData))
            return;
    ...
    private void OnRemoveMemberFromSquad(EntityUid uid, SecTrackComponent component, RemoveMemberFromSquadMessage msg)
    {
        if (component.Station == null || !_stationData.TryGetValue(component.Station.Value, out var stationData))
            return;
    ...
    private void OnChangeSquadStatus(EntityUid uid, SecTrackComponent component, ChangeSquadStatusMessage msg)
    {
        if (component.Station == null || !_stationData.TryGetValue(component.Station.Value, out var stationData))
            return;
    ...
    private void OnRemoveTimer(EntityUid uid, SecTrackComponent component, RemoveTimerMessage msg)
    {
    ...
    private void UpdateUi(EntityUid uid, SecTrackComponent comp)
    {
        if (!_ui.HasUi(uid, SecTrackUiKey.Key) || comp.Station == null)
            return;
        ...
        var state = new SecTrackUpdateState(
            stationName,
            securityCrew,
            unassignedSecurity,
            squads
        ...
        _ui.SetUiState(uid, SecTrackUiKey.Key, state);
    }
    ...
    private (string Location, bool HasLocation) GetSquadApproximateLocation(Squad squad, List<CrewMemberInfo> securityCrew)
    {
        ...
        if (trackedPositions.Count == 0)
            return (Loc.GetString("sec-track-unknown"), false);
    ...
    private void UpdateTimerStateForStation(EntityUid station)
    {
        ...
        while (query.MoveNext(out var uid, out var comp))
        {
            if (comp.Station == station && _ui.IsUiOpen(uid, SecTrackUiKey.Key))
            {
