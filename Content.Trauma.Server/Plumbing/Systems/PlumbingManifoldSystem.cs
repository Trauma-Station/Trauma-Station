...
public sealed partial class PlumbingManifoldSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<FluidPipeManifoldComponent, ComponentInit>(OnCompInit);
        //SubscribeLocalEvent<GasPipeManifoldComponent, GasAnalyzerScanEvent>(OnAnalyzed);
    }

    private void OnCompInit(Entity<FluidPipeManifoldComponent> ent, ref ComponentInit args)
    {
        ...
        {
            if (!_nodeContainer.TryGetNode(nodeContainer, inletName, out PlumbingNode? inlet))
                continue;

            foreach (var outletName in ent.Comp.OutletNames)
            {
                if (!_nodeContainer.TryGetNode(nodeContainer, outletName, out PlumbingNode? outlet))
                    continue;
    ...
    //    {
    //        if (!_nodeContainer.TryGetNode(nodeContainer, pipeName, out PlumbingNode? pipe))
    //            continue;
