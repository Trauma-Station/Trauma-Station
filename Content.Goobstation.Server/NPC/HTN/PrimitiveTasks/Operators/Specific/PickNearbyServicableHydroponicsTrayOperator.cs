...
public sealed partial class PickNearbyServicableHydroponicsTrayOperator : HTNOperator
{
        ...
        CancellationToken cancelToken)
    {
        ...
        foreach (var target in _lookup.GetEntitiesInRange(owner, range))
        {
            ...
            if (plantHolderComponent is { WaterLevel: >= PlantbotServiceOperator.RequiredWaterLevelToService, WeedLevel: <= PlantbotServiceOperator.RequiredWeedsAmountToWeed, Harvest: false } && (!emagged || plantHolderComponent.Dead || plantHolderComponent.WaterLevel <= 0f))
                continue;
