...
public sealed partial class QuantumConsoleWindow : FancyWindow
{
    ...
    private readonly List<DifficultyOption> _difficultyButtons = new();
    ...
    public void UpdateState(QuantumConsoleBoundUiState state)
    {
        ...
        foreach (var option in _difficultyButtons)
        {
            if (option.Difficulty == BitrunningDifficulty.Extreme)
                option.Button.Visible = state.ExtremeDifficultyUnlocked;
        }
        ...
        PointsValue.Text = Loc.GetString("bitrunning-ui-server-points-badge", ("points", state.ServerPoints.ToString()));
        ClientsHeader.Text = Loc.GetString("bitrunning-ui-clients-header-count", ("count", state.Occupants.ToString()));
        ScannerValue.Text = Loc.GetString("bitrunning-ui-scanner-inline", ("scanner", state.ConnectedPods.ToString()));
    ...
    private void UpdateDomainRow(DomainRowWidgets row, BitrunningDomainListing domain, QuantumConsoleBoundUiState state)
    {
        ...
        row.CostLabel.Text = Loc.GetString("bitrunning-ui-domain-cost", ("cost", domain.Cost.ToString()));
    ...
    {
        foreach (var difficulty in Enum.GetValues<BitrunningDifficulty>())
        {
            var button = new Button
            {
                HorizontalExpand = true,
                ModulateSelfOverride = GetDifficultyColor(difficulty),
            };

            var option = new DifficultyOption(button, difficulty);
            button.OnPressed += _ => ToggleDifficulty(option);
    ...
    private void ToggleDifficulty(DifficultyOption option)
    {
        _selectedDifficulty = _selectedDifficulty == option.Difficulty ? null : option.Difficulty;
    ...
    private void UpdateDifficultySelectionVisuals()
    {
        foreach (var option in _difficultyButtons)
        {
            var baseText = GetDifficultyLocString(option.Difficulty);
            option.Button.Text = _selectedDifficulty == option.Difficulty
                ? Loc.GetString("bitrunning-ui-difficulty-selected", ("name", baseText))
                : baseText;
        }
    }

    private static string GetDifficultyLocString(BitrunningDifficulty difficulty)
    {
    ...
    private sealed record DifficultyOption(Button Button, BitrunningDifficulty Difficulty);
