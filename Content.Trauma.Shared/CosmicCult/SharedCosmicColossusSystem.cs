    ...
    public override void Update(float frameTime)
    {
        ...
        while (colossusQuery.MoveNext(out var ent, out var comp))
        {
            if (comp.AttackCharge && _timing.CurTime >= comp.AttackAnimationTimer)
            {
                ...
                _mobs.Clear();
                _lookup.GetEntitiesInRange(Transform(ent).Coordinates, comp.SunderRange, _mobs);
                _mobs.RemoveWhere(target => _cult.EntityIsCultist(target));
                ...
                _targets.Clear();
                _lookup.GetEntitiesInRange(Transform(ent).Coordinates, comp.SunderRange, _targets);
                _targets.RemoveWhere(target => _cult.EntityIsCultist(target)
                || target.Comp.BodyType == BodyType.Static
                || HasComp<GhostComponent>(target)
                || _container.TryGetOuterContainer(target, Transform(target), out _));
