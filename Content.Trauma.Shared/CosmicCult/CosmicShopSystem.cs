...
public sealed partial class CosmicShopSystem : EntitySystem
{
    ...
    {
        if (!HasComp<CosmicCultistComponent>(args.Actor))
            return;
    ...
    {
        var user = args.Actor;
        if (!_prototype.TryIndex(args.InfluenceProtoId, out var proto) || !TryComp<CosmicCultistComponent>(user, out var cultComp))
            return;
    ...
    {
        if (!TryComp<CosmicCultistComponent>(args.Actor, out var cultComp) || cultComp.RespecsAvailable <= 0)
            return;
