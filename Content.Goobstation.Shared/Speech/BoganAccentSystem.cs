...
public sealed partial class BoganAccentSystem : EntitySystem
{
    ...
    private static readonly ProtoId<ReplacementAccentPrototype> Accent = "bogan";

    private readonly StringBuilder _sb = new();

    [SubscribeLocalEvent]
    private void OnAccent(Entity<BoganAccentComponent> ent, ref AccentGetEvent args)
    {
        _sb.Clear();
        ...
        if (rand.Prob(0.15f))
        {
            var pick = rand.Next(1, 4);

            _sb.Append(Loc.GetString($"accent-bogan-prefix-{pick}"));
            _sb.Append(' ');
        ...
        if (rand.Prob(0.3f))
        {
            ...
            _sb.Append(Loc.GetString($"accent-bogan-suffix-{pick}"));
        }
