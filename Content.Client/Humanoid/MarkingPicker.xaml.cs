...
public sealed partial class MarkingPicker : Control
{
    ...
    public void PopulateUsed()
    {
        ...
        foreach (var marking in _currentMarkings.GetReverseEnumerator(_selectedMarkingCategory))
        {
            ...
            if (GetMarkingTexture(newMarking) is not { } texture) // Trauma
                continue;
    ...
    private void OnUsedMarkingSelected(ItemList.ItemListSelectedEventArgs item)
    {
        ...
        for (int i = 0; i < prototype.ColorCount; i++) // Trauma edit
        {
            ...
            // Trauma edit start
            if (_markingManager.GetMarkingShaderParams?.Invoke(prototype, i, _currentMarkings) is { } shaderParams)
            {
                ...
                Color GetParamColor()
                {
                    var list = sliders.Select(x => x.Value).ToList();
                    var vec = Vector4.Zero;

                    for (var j = 0; j < list.Count && j < 4; j++)
                    {
                        vec[j] = list[j];
                    }
    ...
    private void MarkingAdd()
    {
        ...
        if (GetMarkingTexture(marking) is not { } texture) // Trauma
            return;
        ...
        if (MarkingCategoriesExtension.IgnoresMatchSkin(marking.MarkingCategory) || // Trauma edit
    ...
    public Texture? GetMarkingTexture(MarkingPrototype marking) // Trauma
    {
