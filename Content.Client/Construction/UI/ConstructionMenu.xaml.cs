        ...
        public List<string> GetSkills(ConstructionPrototype proto, IPrototypeManager protoMan)
        {
            ...
            if (proto.Practical is { })
            {
                ...
                foreach (var (id, amount) in proto.Practical)
                {
                    ...
                    if (!foundFirst)
                    {
                        if (!prototype.Components.ContainsKey("Knowledge"))
                            continue;

                        var knowledge = (KnowledgeComponent) prototype.Components["Knowledge"].Component;

                        if (knowledge.Category != "Crafting")
                            continue;

                        requirements.Add(Loc.GetString("construction-menu-requirement-main-skill", ("name", name), ("amount", amount)));
                        foundFirst = true;
                        continue;
                    }
